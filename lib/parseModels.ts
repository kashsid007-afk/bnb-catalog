...
const BRAND_PATTERNS: { pattern: RegExp; brand: string }[] = [
  ...
  { pattern: /iphone|apple/i,    brand: 'Apple'      },
...
export function parseBroadcastText(text: string): ModelMap {
  ...
  for (const line of lines) {
    ...
    if (brandMatch && (line.includes(':') || line.includes('👉') || /^[a-z0-9 /+]+$/i.test(line))) {
    ...
    if (model.length >= 1 && model.length <= 30) {
      if (!result[currentBrand].some(existing => existing.toLowerCase() === model.toLowerCase())) {
